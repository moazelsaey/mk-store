...
(function () {
  ...
  document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (item.dataset.page === 'categories') { renderCategoriesPage(); }
    });
  });
  ...
  function populateCategoryFilter(categories) {
    ...
    // Product form dropdown - preserve current selection
    if (pCatSel) {
      const current = pCatSel.value;
      pCatSel.innerHTML = cats.map(c => `<option value="${c}"${c===current?' selected':''}>${c}</option>`).join('');
    }
  }
  ...
  function renderCategoriesList() {
    ...
    const defaultCats = ['V-Bucks','Outfits','Emotes','Pickaxes','Gliders','Wraps','Bundles','Battle Pass'];
    if (!allCategories.length) {
      list.innerHTML = '<p style="color:var(--muted);padding:1rem 1.25rem;font-size:.82rem">No custom categories yet.</p>';
      return;
    }
    list.innerHTML = allCategories.map(cat => {
      const isDefault = defaultCats.includes(cat);
      return `<div style="display:flex;align-items:center;justify-content:space-between;padding:.7rem 1.25rem;border-bottom:1px solid var(--border)">
        <div style="display:flex;align-items:center;gap:.6rem">
          <span style="font-weight:700;font-size:.88rem">${cat}</span>
          ${isDefault ? '<span class="badge" style="font-size:.58rem;padding:1px 6px">Default</span>' : ''}
        </div>
        <div style="display:flex;gap:.5rem">
          ${!isDefault ? `<button class="btn btn-ghost btn-sm rename-cat-btn" data-cat="${cat}">✏️ Rename</button>
          <button class="btn btn-danger btn-sm delete-cat-btn" data-cat="${cat}">🗑 Delete</button>` : '<span style="font-size:.72rem;color:var(--muted)">Protected</span>'}
        </div>
      </div>`;
    }).join('');

    list.querySelectorAll('.rename-cat-btn').forEach(btn => {
      btn.addEventListener('click', () => openRenameCat(btn.dataset.cat));
    });
  ...
  if (addCatBtn) {
    addCatBtn.addEventListener('click', async () => {
      ...
      try {
        const data = await api('/api/admin/categories', 'POST', { name });
        allCategories = data.categories;
        renderCategoriesList();
        input.value = '';
  ...
  async function deleteCat(name) {
    if (!confirm(`Delete category "${name}"?\n\nProducts in this category will keep their category label but it won't appear in filters.`)) return;
    try {
      const data = await api(`/api/admin/categories/${encodeURIComponent(name)}`, 'DELETE');
      allCategories = data.categories;
      renderCategoriesList();
  ...
  // Rename category
  function openRenameCat(name) {
    document.getElementById('renameCatOld').value   = name;
    document.getElementById('renameCatInput').value = name;
    document.getElementById('renameCatOverlay').classList.add('open');
  }
  document.getElementById('renameCatClose')?.addEventListener('click',  () => document.getElementById('renameCatOverlay').classList.remove('open'));
  document.getElementById('renameCatClose2')?.addEventListener('click', () => document.getElementById('renameCatOverlay').classList.remove('open'));
  document.getElementById('renameCatSave')?.addEventListener('click', async () => {
    const oldName = document.getElementById('renameCatOld').value;
    const newName = document.getElementById('renameCatInput').value.trim();
    if (!newName) { toast('Enter a name', 'error'); return; }
    try {
      const data = await api(`/api/admin/categories/${encodeURIComponent(oldName)}`, 'PUT', { name: newName });
      allCategories = data.categories;
      renderCategoriesList();
      document.getElementById('renameCatOverlay').classList.remove('open');
